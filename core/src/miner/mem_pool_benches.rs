...
use super::mem_pool::{AccountDetails, MemPool, PoolingInstant, TxOrigin, TxTimelock};
use crate::transaction::SignedTransaction;

const NUM_TXS: usize = 500;
...
pub fn add_in_ascending_order(bencher: &mut Bencher) {
    ...
    let inputs = &inputs;
    bencher.iter(|| {
        let mut mem_pool = MemPool::with_limits(10000, usize::max_value());
        for input in inputs {
            mem_pool.add(input.transaction.clone(), input.origin, current_time, current_timestamp, input.timelock, &fetch_account).unwrap();
        }
        black_box(mem_pool);
    });
}
...
pub fn add_then_remove_in_ascending_order(bencher: &mut Bencher) {
    ...
    bencher.iter(|| {
        ...
        for input in inputs {
            mem_pool.remove(&input.transaction.hash(), &fetch_seq, current_time, current_timestamp);
        }
        black_box(mem_pool);
    });
}
...
pub fn add_then_remove_old(bencher: &mut Bencher) {
    ...
    bencher.iter(|| {
        let mut mem_pool = MemPool::with_limits(10000, usize::max_value());
        for input in inputs {
            mem_pool.add(input.transaction.clone(), input.origin, current_time, current_timestamp, input.timelock, &old_fetch_account).unwrap();
        }
