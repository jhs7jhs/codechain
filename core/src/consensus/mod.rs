...
pub enum EngineError {
    ...
    /// The height or the signer index of message does not belong to an authority.
    MessageNotAuthorized(u64, usize, Address),
    /// The validator on the given height and index is from the future or does not exist.
    ValidatorNotExist(u64, usize),
...
impl fmt::Display for EngineError {
    fn fmt(&self, f: &mut fmt::Formatter) -> fmt::Result {
        ...
        let msg = match self {
            ...
            MessageNotAuthorized(height, index, address) => {
                format!("The {}th validator({}) on height {} is not authorized.", index, address, height)
            }
            ValidatorNotExist(height, index) => format!(
